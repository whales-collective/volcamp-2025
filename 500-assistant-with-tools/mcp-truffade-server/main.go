...
func main() {
	...
	// -------------------------------------------------
	errEmbedding := GenerateEmbeddings(ctx, &client, "truffade_agent", helpers.GetEnvOrDefault("TRUFFADE_AGENT_KNOWLEDGE_BASE_PATH", ""))
	if errEmbedding != nil {
		fmt.Println("🔶 Error generating embeddings for truffade agent:", errEmbedding)
	}

	// =================================================
	// TOOLS:
	// =================================================
	truffadeTool := mcp.NewTool("search_information_about_truffade",
		mcp.WithDescription(`Search for information about Truffade in the knowledge base.`),
		mcp.WithString("content",
			mcp.Required(),
			mcp.Description("Content to search for similarities."),
		),
	)
	s.AddTool(truffadeTool, truffadeHandler(&client))

	truffadeToolFrench := mcp.NewTool("trouver_des_informations_sur_la_truffade",
		mcp.WithDescription(`Rechercher des informations sur la Truffade dans la base de connaissances.`),
		mcp.WithString("content",
			mcp.Required(),
			mcp.Description("Content to search for similarities."),
		),
	)
	s.AddTool(truffadeToolFrench, truffadeHandler(&client))
...
func truffadeHandler(client *openai.Client) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	similarities, err := SearchSimilarities(ctx, client, "truffade_agent", content, threshold, topN)
	if err != nil {
